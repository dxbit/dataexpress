...
uses
  ...
  Grids, ButtonPanel, Menus, strconsts, SynEditMarks;
...
begin
  if BreakpointsFm = nil then
  	BreakpointsFm := TBreakpointsFm.Create(nil);
...
begin
  if Confirm(rsDeletingBreakpoint, rsAreYouSure) = mrNo then Exit;
...
var
  i: Integer;
begin
  if Confirm(rsDeletingAllBreakpoints, rsAreYouSure) = mrNo then Exit;
...
var
  ...
  begin
    ...
    begin
      S := Grid.Cells[0, i];
      n := UTF8CompareText(ModuleName, S);
      if n < 0 then
      begin
        r := i; Break;
      end
