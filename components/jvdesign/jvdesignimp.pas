...
type
  ...
  private
    ...
    // 7bit //
    FCapturedContainer: TControl;
    procedure SelectControls(aRect: TRect);
    //
  protected
...
begin
  ...
  {$else}  // 7bit
    P := Selected.Parent.ClientToScreen(Selected.BoundsRect.TopLeft);
    P := Container.ScreenToClient(P);
  {$endif} //
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // 7bit //
        if (Fclicked <> nil) and (csNoDesignSelectable in FClicked.ControlStyle) then
          FClicked := nil;
        //
  ...
  begin
    case FDragMode of
      dmSelect, dmCreate:
        begin
          {if GetKeyShiftState * [ssCtrl] = [] then
            Surface.ClearSelection;  }
          Surface.ClearSelection;
          FCapturedContainer := Surface.FindControl(X, Y);
      ...
      dmMove:
        begin
          ...
          begin
            // 7bit
            if Surface.Selector.IsSelected(FClicked) then
              Surface.Selector.RemoveFromSelection(FClicked)
            else
            //
...
procedure TJvDesignController.SelectControls(aRect: TRect);
var
  i: Integer;
  R: TRect;
  C: TControl;
...
begin
  if FCapturedContainer = nil then
    Cnt := Surface.Container
  else
    if (FCapturedContainer is TWinControl) and
      (csAcceptsControls in FCapturedContainer.ControlStyle) then
      Cnt := TWinControl(FCapturedContainer)
    else
      Cnt := TWinControl(FCapturedContainer).Parent;
  //Cnt := Surface.SelectedContainer;
  for i := 0 to Cnt.ControlCount - 1 do
  begin
    C := Cnt.Controls[i];
    P := C.ClientToParent(Point(0, 0), Surface.Container);
    R := Rect(P.X, P.Y, P.X + C.Width, P.Y + C.Height);
    if (IntersectRect(R, R, aRect)) and (C.ControlStyle * [csNoDesignSelectable] = []) then
      Surface.Selector.AddToSelection(C);
  end;
  if Length(Surface.Selected) > 1 then
    Surface.Selector.RemoveFromSelection(Cnt);
end;
