...
type
  ...
  private
    ...
    function GetShadowOffset: TBGRAPointF;
    ...
    procedure FillPoly(const points: array of TBGRAPointF);
    procedure FillStrokePoly(const points: array of TBGRAPointF; fillOver: boolean);
    ...
    procedure SetShadowOffset(const AValue: TBGRAPointF);
    ...
    procedure StrokePoly(const points: array of TBGRAPointF);
    procedure DrawShadow(const points, points2: array of TBGRAPointF);
    procedure ClearPoly(const points: array of TBGRAPointF);
    function ApplyTransform(const points: array of TBGRAPointF; matrix: TAffineMatrix): ArrayOfTPointF; overload;
    function ApplyTransform(const points: array of TBGRAPointF): ArrayOfTPointF; overload;
    function ApplyTransform(point: TBGRAPointF): TBGRAPointF; overload;
    function GetPenPos(defaultX, defaultY: single): TBGRAPointF;
    function GetPenPos(defaultPt: TBGRAPointF): TBGRAPointF;
    procedure AddPoint(point: TBGRAPointF);
    procedure AddPoints(const points: array of TBGRAPointF);
    procedure AddPointsRev(const points: array of TBGRAPointF);
  ...
  public
    ...
    function createLinearGradient(p0,p1: TBGRAPointF): IBGRACanvasGradient2D; overload;
    function createLinearGradient(x0,y0,x1,y1: single; Colors: TBGRACustomGradient): IBGRACanvasGradient2D; overload;
    function createLinearGradient(p0,p1: TBGRAPointF; Colors: TBGRACustomGradient): IBGRACanvasGradient2D; overload;
    ...
    procedure moveTo(const pt: TBGRAPointF); overload;
    procedure lineTo(const pt: TBGRAPointF); overload;
    procedure polylineTo(const pts: array of TBGRAPointF);
    procedure quadraticCurveTo(cpx,cpy,x,y: single); overload;
    procedure quadraticCurveTo(const cp,pt: TBGRAPointF); overload;
    procedure bezierCurveTo(cp1x,cp1y,cp2x,cp2y,x,y: single); overload;
    procedure bezierCurveTo(const cp1,cp2,pt: TBGRAPointF); overload;
    ...
    procedure spline(const pts: array of TBGRAPointF; style: TSplineStyle= ssOutside);
    procedure splineTo(const pts: array of TBGRAPointF; style: TSplineStyle= ssOutside);
    ...
    procedure arcTo(p1,p2: TBGRAPointF; radius: single); overload;
    ...
    function isPointInPath(pt: TBGRAPointF): boolean; overload;
    ...
    property shadowOffset: TBGRAPointF read GetShadowOffset write SetShadowOffset;
...
type
  ...
  public
    ...
    constructor Create(p0,p1: TBGRAPointF);
  end;
  ...
  public
    function getTexture: IBGRAScanner; override;
    constructor Create(source: TBGRACustomBitmap; repeatX,repeatY: boolean; Origin, HAxis, VAxis: TBGRAPointF);
...
constructor TBGRACanvasPattern2D.Create(source: TBGRACustomBitmap; repeatX,
  repeatY: boolean; Origin, HAxis, VAxis: TBGRAPointF);
var
...
constructor TBGRACanvasLinearGradient2D.Create(p0, p1: TBGRAPointF);
begin
  o1 := p0;
  o2 := p1;
end;
...
function TBGRACanvas2D.GetShadowOffset: TBGRAPointF;
begin
...
procedure TBGRACanvas2D.FillPoly(const points: array of TBGRAPointF);
var
...
procedure TBGRACanvas2D.FillStrokePoly(const points: array of TBGRAPointF;
  fillOver: boolean);
var
  tempScan,tempScan2: TBGRACustomScanner;
  multi: TBGRAMultishapeFiller;
  contour : array of TBGRAPointF;
...
procedure TBGRACanvas2D.SetShadowOffset(const AValue: TBGRAPointF);
begin
...
procedure TBGRACanvas2D.StrokePoly(const points: array of TBGRAPointF);
var
  texture: IBGRAScanner;
  tempScan: TBGRACustomScanner;
  contour: array of TBGRAPointF;
...
procedure TBGRACanvas2D.DrawShadow(const points, points2: array of TBGRAPointF);
var ofsPts,ofsPts2: array of TBGRAPointF;
    offset: TBGRAPointF;
    i: Integer;
    ...
    procedure AddPt(const coord: TBGRAPointF);
    var pixRect: TRect;
    begin
...
procedure TBGRACanvas2D.ClearPoly(const points: array of TBGRAPointF);
begin
...
function TBGRACanvas2D.ApplyTransform(const points: array of TBGRAPointF;
...
function TBGRACanvas2D.ApplyTransform(point: TBGRAPointF): TBGRAPointF;
begin
  result := currentState.matrix*point+FCanvasOffset;
end;

function TBGRACanvas2D.GetPenPos(defaultX,defaultY: single): TBGRAPointF;
begin
...
function TBGRACanvas2D.GetPenPos(defaultPt: TBGRAPointF): TBGRAPointF;
begin
  result := GetPenPos(defaultPt.x,defaultPt.y);
end;

procedure TBGRACanvas2D.AddPoint(point: TBGRAPointF);
begin
...
procedure TBGRACanvas2D.AddPoints(const points: array of TBGRAPointF);
var i: integer;
begin
...
procedure TBGRACanvas2D.AddPointsRev(const points: array of TBGRAPointF);
var i: integer;
begin
  if FPathPointCount+length(points) > length(FPathPoints) then
    setlength(FPathPoints, max( (length(FPathPoints)+1)*2, FPathPointCount+length(points) ) );
...
function TBGRACanvas2D.createLinearGradient(p0, p1: TBGRAPointF
  ): IBGRACanvasGradient2D;
begin
...
var i,j: integer;
  pts, splinePts: array of TBGRAPointF;
  nb: integer;
begin
  if FPathPointCount > 0 then
  begin
...
procedure TBGRACanvas2D.moveTo(const pt: TBGRAPointF);
begin
  if (FPathPointCount <> 0) and not isEmptyPointF(FPathPoints[FPathPointCount-1]) then
    AddPoint(EmptyPointF);
...
procedure TBGRACanvas2D.lineTo(const pt: TBGRAPointF);
begin
  AddPoint(ApplyTransform(pt));
  FLastCoord := pt;
end;

procedure TBGRACanvas2D.polylineTo(const pts: array of TBGRAPointF);
begin
...
procedure TBGRACanvas2D.quadraticCurveTo(const cp, pt: TBGRAPointF);
begin
...
procedure TBGRACanvas2D.bezierCurveTo(const cp1, cp2, pt: TBGRAPointF);
begin
...
procedure TBGRACanvas2D.spline(const pts: array of TBGRAPointF; style: TSplineStyle);
var transf: array of TBGRAPointF;
begin
...
procedure TBGRACanvas2D.splineTo(const pts: array of TBGRAPointF;
  style: TSplineStyle);
var transf: array of TBGRAPointF;
  i: Integer;
begin
...
var pts: array of TBGRAPointF;
  temp: single;
  pt: TBGRAPointF;
  rx,ry: single;
  len1,len2: single;
  unitAffine: TAffineMatrix;
  v1orig,v2orig,v1ortho,v2ortho: TBGRAPointF;
...
procedure TBGRACanvas2D.arcTo(p1, p2: TBGRAPointF; radius: single);
begin
...
function TBGRACanvas2D.isPointInPath(pt: TBGRAPointF): boolean;
begin
