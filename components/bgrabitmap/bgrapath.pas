...
type
  ...
  protected
    ...
    FLastCoord,
    FStartCoord: TBGRAPointF;
    FExpectedControlPoint: TBGRAPointF;
    ...
    procedure StoreCoord(const pt: TBGRAPointF);
    function ReadCoord: TBGRAPointF;
  ...
  public
    ...
    procedure rotate(angleRadCW: single; center: TBGRAPointF); overload;
    procedure rotateDeg(angleDeg: single; center: TBGRAPointF); overload;
    ...
    procedure smoothQuadraticCurveTo(const pt: TBGRAPointF); overload;
    ...
    procedure smoothBezierCurveTo(const cp2,pt: TBGRAPointF); overload;
    ...
    procedure arcTo(const p1,p2: TBGRAPointF; radius: single); overload;
...
function ComputeClosedSpline(const points: array of TBGRAPointF; Style: TSplineStyle): ArrayOfTPointF;
function ComputeOpenedSpline(const points: array of TBGRAPointF; Style: TSplineStyle; EndCoeff: single = 0.25): ArrayOfTPointF;
...
function Html5ArcTo(const p0, p1, p2: TBGRAPointF; radius: single): TArcDef;
function SvgArcTo(const p0: TBGRAPointF; rx, ry, xAngleRadCW: single; largeArc,
  anticlockwise: boolean; const p1: TBGRAPointF): TArcDef;
function ArcStartPoint(const arc: TArcDef): TBGRAPointF;
function ArcEndPoint(const arc: TArcDef): TBGRAPointF;
...
function ComputeCurvePrecision(pt1, pt2, pt3, pt4: TBGRAPointF): integer;
var
  len: single;
begin
...
var
  curves: array of array of TBGRAPointF;
  nb: integer;
  lastPt: TBGRAPointF;
  i: Integer;
  j: Integer;

  procedure AddPt(pt: TBGRAPointF); inline;
  begin
    result[nb]:= pt;
    inc(nb);
    lastPt := pt;
  ...
  function EqLast(pt: TBGRAPointF): boolean;
  begin
...
var
  ...
  ptPrev, ptPrev2, ptNext, ptNext2: TBGRAPointF;
  t: single;
...
function ComputeOpenedSpline(const points: array of TBGRAPointF; Style: TSplineStyle; EndCoeff: single): ArrayOfTPointF;
var
  i, j, nb, idx, pre: integer;
  ptPrev, ptPrev2, ptNext, ptNext2: TBGRAPointF;
  t: single;
...
var q0,q1,q2,q3,q4: array of TBGRAPointF;
  temp: Single;
begin
  if x1 > x2 then
  begin
...
function Html5ArcTo(const p0, p1, p2: TBGRAPointF; radius: single
  ): TArcDef;
var p3,p4,an,bn,cn,c: TBGRAPointF;
...
var savedPos: integer;
  ...
  function CoordToString(const pt: TBGRAPointF): string;
  begin
    lastPos := pt;
...
var p: integer;
  ...
  function parseCoord(relative: boolean): TBGRAPointF;
  begin
...
procedure TBGRAPath.StoreCoord(const pt: TBGRAPointF);
begin
...
function TBGRAPath.ReadCoord: TBGRAPointF;
begin
  result := PBGRAPointF(FData+FDataPos)^;
  inc(FDataPos,sizeof(TBGRAPointF));
end;
...
procedure TBGRAPath.rotate(angleRadCW: single; center: TBGRAPointF);
begin
...
procedure TBGRAPath.rotateDeg(angleDeg: single; center: TBGRAPointF);
begin
  translate(center.x,center.y);
  rotateDeg(angleDeg);
  translate(-center.x,-center.y);
...
procedure TBGRAPath.moveTo(const pt: TBGRAPointF);
begin
...
procedure TBGRAPath.lineTo(const pt: TBGRAPointF);
begin
...
procedure TBGRAPath.polylineTo(const pts: array of TBGRAPointF);
var i: integer;
begin
...
procedure TBGRAPath.quadraticCurveTo(const cp, pt: TBGRAPointF);
begin
  if not isEmptyPointF(FLastCoord) then
  begin
...
procedure TBGRAPath.bezierCurveTo(const cp1, cp2, pt: TBGRAPointF);
begin
...
procedure TBGRAPath.smoothBezierCurveTo(const cp2, pt: TBGRAPointF);
begin
...
procedure TBGRAPath.smoothQuadraticCurveTo(const pt: TBGRAPointF);
begin
...
procedure TBGRAPath.arcTo(const p1, p2: TBGRAPointF; radius: single);
var p0 : TBGRAPointF;
begin
