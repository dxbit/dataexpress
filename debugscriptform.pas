...
uses
  Classes, SysUtils, Types, FileUtil, SynEdit, SynHighlighterPas, Forms,
  Controls, Graphics, Dialogs, ComCtrls, strconsts, scriptmanager,
  SynEditMiscClasses, uPSDebugger, uPSUtils, uPSRuntime, scriptedit,
  LclType, Menus;
...
begin
  ...
  begin
    ...
    Result := PSVariantToString(Tmp, s);
...
begin
  ...
  begin
  	TRunScript(FExec.Id).AddBreakpoints;
  end;
end;
...
begin
  AppConfig.DebugFormWidth:=Width;
  AppConfig.DebugFormHeight:=Height;
...
begin
  ...
  Edit.CurLine := aLine;
  FExec.Pause;
  //ScriptMan.DebugMode:=dmPaused;
  Edit.CaretY:=aLine;
  Edit.CaretX := 1;
  if aLine > Edit.TopLine + Edit.LinesInWindow - 4 then
  	Edit.TopLine := aLine - Edit.LinesInWindow div 2;

  //OutputFm.AddMsg(IntToStr(aLine) + ': ' + IntToStr(Edit.TopLine + Edit.LinesInWindow - 4) + ': ' + IntToStr(Edit.TopLine) );
