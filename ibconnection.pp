...
begin
  ...
  with cursor as TIBCursor do
    ...
    for i := 0 to SQLDA^.SQLD - 1 do
      ...
      // 7bit Кодировка полей в базе данных DataExpress всегда UTF8. Я закомментировал
      // этот участок, т. к. при запросе select с blob-полями, возникает
      // ошибка 335545033 при вызове isc_blob_lookup_desc в функции GetBlobCharset.
      // Ошибка возникает только при условии подключения старого клиента 2.5 к
      // новому серверу Firebird 5.
