...
type
  ...
  private
    ...
    function UsersExists: Boolean;
    procedure CreateUsers;
    function IntfsExists: Boolean;
    procedure CreateIntfs;
    function ConnExists: Boolean;
    procedure CreateConn;
    function ConnIdExists: Boolean;
    procedure CreateConnID;
...
uses
  LazUtf8, sqldb, dbengine, apputils, SAX, saxbasereader, Db, dxctrls, mytypes;
...
begin
  ...
  begin
    ...
    MI.Caption := Atts.GetValue('', 'caption');
...
begin
  Result := nil;
  for i := 0 to Count - 1 do
    if Utf8CompareText(Intfs[i].Name, aName) = 0 then Exit(Intfs[i]);
end;
...
var
  i: Integer;
begin
  S := S + '<menuitem caption="' + MI.Caption + '" kind="' + IntToStr(Ord(MI.Kind)) +
...
begin
  S := S + '<interface id="' + IntToStr(Intf.Id) + '" name="' + Intf.Name +
...
begin
  ...
  begin
    U := FUsers.AddUser;
    U.Id := GetInt(Atts, 'id');
    U.Name := Atts.GetValue('', 'name');
    U.Password:=Atts.GetValue('', 'password');
...
function TdxUserManager.UsersExists: Boolean;
var
  DS: TSQLQuery;
begin
  try
    DS := DBase.OpenDataSet('select id from dx_users');
    Result := True;
    DS.Free;
  except
    Result := False;
...
procedure TdxUserManager.CreateUsers;
begin
  DBase.Execute('CREATE TABLE DX_USERS (ID INTEGER, USERS BLOB SUB_TYPE 1 SEGMENT SIZE 80,' +
    'ROLES BLOB SUB_TYPE 1 SEGMENT SIZE 80, INTFS BLOB SUB_TYPE 1 SEGMENT SIZE 80);');
end;

function TdxUserManager.IntfsExists: Boolean;
var
  DS: TSQLQuery;
begin
  try
    DS := DBase.OpenDataSet('select intfs from dx_users');
    Result := True;
    DS.Free;
  except
    Result := False;
...
procedure TdxUserManager.CreateIntfs;
begin
  DBase.Execute('ALTER TABLE DX_USERS ADD INTFS BLOB SUB_TYPE 1 SEGMENT SIZE 80;');
end;

function TdxUserManager.ConnIdExists: Boolean;
var
  DS: TSQLQuery;
begin
  try
    DS := DBase.OpenDataSet('select cid from dx_lock');
    DS.Free;
    Result := True;
  except
    Result := False;
...
procedure TdxUserManager.CreateConnID;
begin
  DBase.Execute('ALTER TABLE DX_LOCK ADD CID INTEGER;');
end;

function TdxUserManager.ConnExists: Boolean;
var
  DS: TSQLQuery;
begin
  try
    DS := DBase.OpenDataSet('select id from dx_conn');
    Result := True;
    DS.Free;
  except
    Result := False;
...
procedure TdxUserManager.CreateConn;
begin
  DBase.Execute('CREATE TABLE DX_CONN (ID INTEGER, UID INTEGER, MODE INTEGER, DTIME TIMESTAMP, SID INTEGER);');
end;

function TdxUserManager.LockExists: Boolean;
var
  DS: TSQLQuery;
begin
  try
    DS := DBase.OpenDataSet('select uid from dx_lock');
    Result := True;
    DS.Free;
  except
    Result := False;
...
procedure TdxUserManager.CreateLock;
begin
  DBase.Execute('CREATE TABLE DX_LOCK (CID INTEGER, UID INTEGER, FMID INTEGER, RECID INTEGER, DTIME TIMESTAMP);');
end;
...
var
  ...
  St: TStream;
begin
  //DBase.Execute('delete from dx_users;');

  St := nil;
  DS := DBase.OpenSysTable('select id, users, roles, intfs from dx_users', 'dx_users');
  try
    while not DS.Eof do
    	DS.Delete;
...
begin
  ...
  FS := nil;
  try
    FS := TFileStream.Create(aDir + 'users', fmOpenRead);
    FUsers.LoadFromStream(FS);
    FreeAndNil(FS);
    FS := TFileStream.Create(aDir + 'roles', fmOpenRead);
...
procedure TdxUserManager.CheckUsers;
begin
  if not UsersExists then CreateUsers
  else if not IntfsExists then CreateIntfs;
  if not ConnExists then CreateConn;
  if not LockExists then CreateLock
  else if not ConnIdExists then CreateConnID;
end;
...
{function GetMaxId(DS: TDataSet): Integer;
begin
  Result := 0;
  DS.First;
  while not DS.Eof do
  begin
    if DS.Fields[0].AsInteger > Result then
...
begin
  ...
  DS := DBase.OpenDataSet(S, 'DX_CONN');
  try
...
begin
  ...
  begin
    U := Users[i];
    if Utf8CompareText(aName, U.Name) = 0 then Exit(U);
  end;
end;
...
begin
  ...
  begin
    U := Users[i];
    S := S + '<user id="' + IntToStr(U.Id) + '" name="' + U.Name +
      '" password="' + U.Password + '" roleid="' + IntToStr(U.RoleId) +
      '" singlemode="' + Bool2Str(U.AllowSingleMode) + '" multiauth="' + Bool2Str(U.AllowMultiAuth) + '"/>';
  end;
  S := S + '</users>';
  St.WriteBuffer(Pointer(S)^, Length(S));
end;
...
begin
  ...
  begin
    R := Roles[i];
    if UTF8CompareText(aName, R.Name) = 0 then Exit(R);
  end;
end;
...
begin
  ...
  begin
    R := Roles[i];
    S := S + '<role id="' + IntToStr(R.Id) + '" name="' + R.Name +
