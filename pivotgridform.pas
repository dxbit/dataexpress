...
uses
  ...
  MemDS, DB, PivotGrid, dxreports, strconsts;
...
type
  ...
  public
    { public declarations }
    //procedure ShowForm(RD: TReportData);
    procedure ShowForm(aGrid: TdxPivotGrid);// overload;
  end;
...
uses
  apputils, selectform, dxctrls, fontform, reportmanager,
  pivotfieldform, helpform, mydialogs;
...
begin
  if FontFm.ShowForm(FixFnt.Font) = mrOk then
  begin
...
begin
  if FontFm.ShowForm(Fnt.Font) = mrOk then
  begin
...
begin
  ...
  if PivotFieldFm.ShowForm(N.Text, Col[N.Index], Grid) = mrOk then
    BuildGrid;
end;
...
begin
  ...
  begin
    if Tree.Items.FindNodeWithData(Pointer(i+1)) = nil then
      SL.AddObject(FRD.Grid.Columns[i].Caption, TObject(i+1));
  end;
  SL.Sort;  }
  //if SelectFm.ShowForm(rsFields, '', SL) = mrOk then
  i := ShowSelectQFieldForm(FRD);
  if (i >= 0) and (Tree.Items.FindNodeWithData(Pointer(i+1)) = nil) then
  begin
    //i := Integer(SL.Objects[SelectFm.Index])-1;
    N := Tree.Selected;
    ...
    begin
      i := StrToInt(Copy(Col.FieldName, 2, 100));
      //pF := FRD.Sources[0]^.Fields.FindField(i);
      pF := FRD.FindField(i);
    end;
    if pF <> nil then FI.DataType:=GetLowField(pF)^.Tp
    else FI.DataType := flText;   }
    if FI.DataType in [flObject, flBool, flCounter] then FI.DataType := flNumber;
    RefreshDS;
    BuildGrid;
  end;
...
begin
  ...
  S := FRD.Grid.Columns[i].FieldName;
...
begin
  if FontFm.ShowForm(SelectFnt.Font) = mrOk then
  begin
...
begin
  if FontFm.ShowForm(TotalFixFnt.Font) = mrOk then
  begin
...
begin
  if FontFm.ShowForm(TotalFnt.Font) = mrOk then
  begin
...
begin
  ...
  begin
    ...
    DS.FieldDefs.Add(FRD.Grid.Columns[idx].FieldName, ftString, 200);
  end;
  DS.CreateTable;
  DS.Open;
  DS.Insert;
...
{procedure TPivotGridFm.ShowForm(RD: TReportData);
begin
  FRD := RD;
