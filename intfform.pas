...
uses
  formmanager, reportmanager, apputils, selectform, helpform;
...
begin
  ...
  begin
    ...
    MI.Kind := miForm;
    MI.Id := Fm.Id;
    N := Mnu.Items.AddChild(PN, Fm.FormCaption);
    N.ImageIndex := 7;
    N.SelectedIndex:=7;
    N.Data := MI;
    PN.Expand(False);
...
begin
  ...
  if SelectFm.ShowForm(rsSelectReport, '', SL) = mrOk then
  begin
